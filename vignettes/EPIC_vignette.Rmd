...
Note that users only need to run this entire section 2 once for each trait and apply to different transcriptomic profiles. 
...
We download LDL GWAS summmary statistics from [GLGC](http://csg.sph.umich.edu/willer/public/lipids2013/) [@Willer2013] and use it for demonstration in this vignette. 
```{r}
...
For tissue-specific bulk RNA-seq data from the Genotype-Tissue Expression project [@GTEx2020] ([GTEx v8](http://www.gtexportal.org/)), tissues with fewer than 100 samples were removed from the analysis. After sample-specific quality controls, we obtained gene expression profiles of 45 tissues. For subsequent analyses, we focused on a set of 8,708 genes with tissue specificity scores greater than 5. We then perform log2 transformation on the tissue-specific TPM measurements with an added pseudo-count. We demonstrate the pre-stored GTEx v8 dataset `gtex` in this vignette.
